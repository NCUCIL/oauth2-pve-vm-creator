...
export class OAuth2Strategy extends PassportStrategy(Strategy, "oauth2") {
  ...
  userProfile(accessToken: string, done: (err?: Error | null, portalUser?: PortalUser) => void): void {
  ...
  async validate(accessToken: string, refreshToken: string, portalUser: PortalUser) {
    ...
    if (!user) {
      if (!(portalUser.identifier && portalUser.email && portalUser.studentId))
        throw new BadRequestException("Missing required fields (need id, identifier and studentId), get: " + JSON.stringify(portalUser));
      user = await this.userService.createUser(portalUser.identifier, portalUser.email, portalUser.studentId);
    }
    return user;
  }
}
